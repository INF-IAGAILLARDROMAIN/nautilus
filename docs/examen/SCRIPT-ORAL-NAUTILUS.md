...
« Bonjour, je m'appelle **Romain Gaillard**, j'ai 28 ans, et je vais vous présenter le projet **Nautilus** — un logiciel de gestion d'atelier nautique enrichi d'un agent IA en langage naturel.

Je passe aujourd'hui le **Titre Professionnel Développeur Web et Web Mobile**, RNCP 37674, dans le cadre du parcours **Graduate Développeur IA** que j'ai suivi chez **Studi**.
...
J'ai été **mécanicien nautique pendant une dizaine d'années**, spécialisé en moteurs hors-bord. J'ai travaillé dans plusieurs ateliers **du Morbihan**, j'ai vu les chefs d'atelier au quotidien, j'ai vécu leurs frictions.

En 2025, j'ai fait une **reconversion vers le développement** via le parcours **Graduate Développeur IA** chez Studi — 114 heures de formation, qui m'amènent à passer le Titre Pro Développeur Web et Web Mobile aujourd'hui. Aujourd'hui, je suis **président de RANKIA**, une SASU que j'ai créée en avril 2026 — c'est une société d'édition de logiciels métier pour les PME.
...
Important à clarifier dès maintenant : **Nautilus en V1 est un proof of concept**. C'est un démonstrateur sérieux de l'agent IA, construit autour d'une vraie brique CRM fonctionnelle — clients, bateaux, devis, ordres de réparation, factures. Mais ce n'est pas encore le produit fini commercialisable.

L'**objectif RANKIA va beaucoup plus loin** : la V2 commerciale, que je porterai après l'examen, ajoutera le multi-tenant, le multi-rôles, le scan OCR de plaque moteur — bref, toutes les briques nécessaires pour qu'un atelier puisse remplacer complètement son outil de gestion actuel par Nautilus.

**Zéro formation requise.** Si vous savez poser une question, vous savez utiliser Nautilus. »

> **[Note pour Romain — défense oral si le jury creuse sur les concurrents]**
> *« Le marché des CRM nautiques compte plusieurs acteurs établis. En V1, Nautilus est un proof of concept — il démontre l'agent IA mais ne prétend pas être une alternative commerciale complète aujourd'hui. La V2 RANKIA portera cette ambition : multi-tenant, multi-rôles, scan OCR — tout ce qu'il faut pour qu'un atelier puisse migrer son système actuel vers Nautilus. Le positionnement frontal, c'est la V2. »*
...
Et le projet a un **double objectif** : valider mon Titre Pro Développeur Web et Web Mobile aujourd'hui, et lancer la **commercialisation V2** dans les mois qui suivent. »
...
### Étape 9 — Page Administration en preview (45 secondes)

> **[Tu cliques sur le menu de gauche → Administration.]**

« Je termine la démo sur la page **Administration**. Elle est livrée en V1 en aperçu désactivé — vous voyez le bouton « Inviter un utilisateur » et l'action « Modifier » qui sont volontairement grisés.

Pourquoi cette page ? Pour démontrer le **modèle d'accès** que j'ai retenu : **3 rôles métier** — Client, Mécano, Chef d'atelier — et une **casquette Admin séparée**, cochable indépendamment du rôle. C'est l'entreprise qui décide qui assume cette casquette.

En V1, je suis le seul utilisateur, chef d'atelier et admin. L'activation back du RBAC — invitations par email, Guards NestJS par rôle, filtrage front conditionnel — est dans la roadmap V2 commerciale RANKIA. L'interface est posée, le système est conçu : il ne manque que le branchement. »
